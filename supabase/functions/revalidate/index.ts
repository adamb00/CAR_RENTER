...
const NEXT_REVALIDATE_URL = Deno.env.get('SUPABASE_NEXT_REVALIDATE_URL');
const REVALIDATE_SECRET = Deno.env.get('SUPABASE_REVALIDATE_SECRET');
...
serve(async (req: Request) => {
  ...
  const carId =
    payload?.carId ?? payload?.record?.id ?? payload?.old_record?.id ?? null;
  const locales = payload?.locales;

  try {
    const res = await fetch(
      `${NEXT_REVALIDATE_URL}?secret=${encodeURIComponent(REVALIDATE_SECRET)}`,
      {
        method: 'POST',
